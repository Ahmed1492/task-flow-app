...
const DeployedTasks = ({ allTasks, getData, setAllTasks, loadingTasks }) => {
  const decodeToken = () => {
    try {
  ...
  useEffect(() => { getData(`${decodeToken().id}/deployed`); }, [getData]);

  return (
    <div className="px-4 py-6 fade-up">
      <PageHeader type="deployed" count={Array.isArray(allTasks) ? allTasks.length : 0} />
